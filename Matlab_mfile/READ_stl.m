...
%% 
if nargin==2
  stlFORMAT = lower(varargin{1}); %varagin{1} = stlfilename.stl, lower:대문자를 소문자로 변환
else
  stlFORMAT = 'auto';
end
...
%   disp('STL type : binary')
end %if

%% Prepare the output arguments
if nargout == 2 %출력 변수의 갯수에 따른 정보 반영
  varargout(1) = {coordNORMALS};
elseif nargout == 3
  varargout(1) = {coordNORMALS};
  varargout(2) = {stlNAME}; %문자열만 반환하며, 'unnamed_object' 로 저장됨
end
